    ...
    <div v-if="ports.length > 0">
      ...
      <ul>
        <li v-for="(port, index) in portData" :key="index">
          ...
          <div>
            Serial Output: {{ serialOutputs[index] }}
          </div>
        </li>
      </ul>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getPreviouslyConnectedPorts() {
      try {
        ...
        for (let i = 0; i < this.ports.length; i++) {
          ...
          console.log(`Port ${i} status: ${this.portData[i].status}`);
        }
      } catch (error) {
    ...
    async checkIfFlashed(port, index) {
      try {
        ...
        while (true) {
          ...
          if (done) {
            console.log(`Port ${index} - Done reading.`);
            break; // Exit the loop if the stream is done
          }

          buffer += decoder.decode(value, { stream: true });

          // Process each line when a newline character is found
          let lines = buffer.split('\n');
          buffer = lines.pop();  // Keep any incomplete line in the buffer

          for (let line of lines) {
            line = line.trim();  // Clean up the line
            console.log(`Received line from port ${index}: ${line}`);
            // Handle the response from the board
            if (this.portData[index].status === 'Checking...') {
              if (line.startsWith("Device name:")) {
                const deviceName = line.split(': ')[1];
                if (deviceName == "writer") {
                  this.writer = port;
                }
                this.portData[index].name = deviceName;
                this.portData[index].status = 'Flashed'; // Update portData with the device name
              } else {
                console.log(`Unexpected response from port ${index}: ${line}`);
                this.portData[index].status = 'Not Flashed';
              }
            }
          }
        }
    ...
    uploadToECUs() {
      ...
      this.ports.forEach((port, index) => {
        ...
        if (this.portData[index].status === 'Flashed') {
          ...
          this.uploadToECU(port, selectedIds);
          // let filteredData = {};
    ...
    startSimulation() {
      // open reader on each port and add message to serialOutputs
      // send each message in canData to the writer
      ...
      this.ports.forEach((port, index) => {
        if (this.portData[index].status === 'Flashed') {
          ...
          reader.read().then(({ value, done }) => {
            if (done || !value) {
              console.log(`Port ${index} - Done reading.`);
              return; // Exit the loop if the stream is done
            }
      ...
      this.canData.forEach((id, data) => {
        this.sendCANMessage(this.writer, `${id}#${data}`);
      });
    },
...
// Server sends all CAN messages to the sender over the serial port
// Sender sends each message over the CAN bus
// Receivers receive messages based on the ID filters 
// Receivers report all messages received backt to the server over the serial port
// Server sends a stop message to all boards over the serial port
</script>